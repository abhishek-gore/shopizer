...
public interface ProductReviewRepository extends JpaRepository<ProductReview, Long> {
	...
	@Query("select p from ProductReview p left join fetch p.descriptions pd left join fetch p.reply pr join fetch p.customer pc join fetch p.product pp where pp.merchantStore = ?1")
	List<ProductReview> findByProductMerchantStore(com.salesmanager.core.model.merchant.MerchantStore store);
